...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function initImageSelection() {
    ...
    processedPresetImages.forEach(image => {
      ...
      img.addEventListener('click', () => {
        ...
        currentImageIdentifier = image.name; // 預設圖片使用其 name 作為標識符
        currentImageDisplayName = image.name; // 預設圖片顯示名稱也是其 name
      ...
      img.addEventListener('dblclick', () => {
        ...
        selectedImage = image.src;
        currentImageIdentifier = image.name; // 使用預設圖片 name
        currentImageDisplayName = image.name;
        document.querySelectorAll('.image-options img.selected').forEach(i => i.classList.remove('selected'));
        img.classList.add('selected');

        const gameSize = getGameSize();
        // 傳遞標識符和顯示名稱給 startGame
  ...
  // --- 修改 initStartGameButton: 傳遞顯示名稱 ---
  function initStartGameButton() {
    document.getElementById('start-game').addEventListener('click', () => {
      if (!selectedMode) { alert('請選擇遊戲模式'); return; }
      const gameSize = getGameSize();
      let imageSourceForGame = selectedImage;
      let identifierForGame = currentImageIdentifier; // 使用當前選擇的標識符
      let displayNameForGame = currentImageDisplayName; // 使用當前選擇的顯示名稱

      if (selectedMode === 'image') {
          const customImageInput = document.getElementById('custom-image');
          // 檢查是否有文件被選中 *且* selectedImage 不是 DataURL (表示未處理過 input change)
          // 或者 currentImageIdentifier 不是 'custom_' 開頭 (表示還沒處理 input change)
          if (customImageInput && customImageInput.files[0] && (!selectedImage || !selectedImage.startsWith('data:'))) {
              ...
              identifierForGame = sanitizeFilenameForKey(file.name); // 從檔名生成標識符
              displayNameForGame = file.name; // 原始檔名作為顯示名稱
              const reader = new FileReader();
              reader.onload = (e) => {
                  imageSourceForGame = e.target.result;
                  // 在這裡再次設置，以防 selectedImage 未更新
          ...
          // 如果已經選擇了圖片 (預設或已處理的自定義)
          else if (selectedImage && identifierForGame) {
              // displayNameForGame 應該已經在選擇時設置好了
              startGame(imageSourceForGame, identifierForGame, gameSize, displayNameForGame);
  ...
  // --- 修改 startGame: 接收並設置 displayName ---
  async function startGame(imageSource, identifier, size, displayName) { // 添加 displayName 參數
    ...
    try {
      ...
      if (gameInstance) {
          // gameInstance.imageIdentifier = identifier; // 由構造函數處理
          // gameInstance.imageDisplayName = displayName; // 由構造函數處理
          renderGameBoard();
  ...
  // --- 修改 gameComplete: 再玩一次時傳遞 displayName ---
  function gameComplete() {
    ...
    gameInstance.saveHighScore(); // 使用 gameInstance 內部的 identifier 和 displayName
    ...
    const playAgainHandler = () => {
        ...
        if (gameInstance) {
            // 重新開始同一關卡，傳遞所有必要信息
            startGame(
                gameInstance.imageSource, // 可能需要重新獲取原始 source 或 DataURL
  ...
  // loadHighScores: 使用 identifier 加載，顯示時依賴 score.levelName
  function loadHighScores(mode, identifier, size) {
      const highScoresList = document.getElementById('high-scores-list');
      if (!highScoresList) return;
      highScoresList.innerHTML = '';
      // 數字模式 identifier 為空，但需要加載
      if (mode === 'number' && size !== undefined && size !== null) {
          identifier = ''; // 確保數字模式用空標識符
      } else if (mode === 'image' && (!identifier || size === undefined || size === null)) {
          highScoresList.innerHTML = '<div class="no-record">參數錯誤無法載入記錄</div>';
          return;
      } else if (mode !== 'number' && mode !== 'image') {
           highScoresList.innerHTML = '<div class="no-record">模式錯誤無法載入記錄</div>';
           return;
      }

      const scores = StorageManager.getItem('puzzleHighScores', {});
      const key = `${mode}-${identifier}-${size}`; // 使用傳入的 identifier 構造 key
      ...
      if (modeScores.length === 0) {
        ...
        modeScores.forEach((score, index) => {
          ...
          levelCell.title = score.levelName || '未知關卡'; // 添加 title 以防名稱過長
          // ---------------------------------
  ...
  // initGameControls: 修復重置計時器問題
  function initGameControls() {
      document.getElementById('reset-game').addEventListener('click', () => {
          if (!gameInstance) return;
          gameInstance.resetGame();
          renderGameBoard();
          gameInstance.startTimer(); // <--- 確保重新啟動計時器
          soundManager.playGameStartSound();
          if (gameInstance) gameInstance.cheatEnabled = false;
          const cheatButton = document.getElementById('cheat-button');
          if (cheatButton) { cheatButton.classList.remove('active'); cheatButton.style.backgroundColor = '#e74c3c'; }
      ...
      if(cheatButton) {
           cheatButton.addEventListener('click', () => {
               if (!gameInstance || !gameInstance.startTime) { // 檢查 startTime
                   alert('遊戲尚未開始或計時器異常');
                   return;
               }
  ...
  // --- 修改 initCustomImageUpload: 處理檔名和標識符 ---
  function initCustomImageUpload() {
    const customInput = document.getElementById('custom-image');
    if (!customInput) return;
    customInput.addEventListener('change', (e) => {
      if (e.target.files && e.target.files[0]) {
        const file = e.target.files[0]; // 獲取文件對象
        const reader = new FileReader();

        reader.onload = async (event) => {
            selectedImage = event.target.result; // 存儲 DataURL
            currentImageIdentifier = sanitizeFilenameForKey(file.name); // 生成清理後的標識符
            currentImageDisplayName = file.name; // 存儲原始檔名用於顯示
            ...
            if (imageOptions) {
                 ...
                 preview.alt = '自定義預覽';
                 preview.title = currentImageDisplayName; // 顯示原始檔名
  ...
  // initWebImageSearch: 也添加雙擊和 displayName
  function initWebImageSearch() {
    ...
    WebImageSearch.init((imageUrl, imageName) => { // imageName 是 Pixabay 的標籤或生成的名稱
      document.querySelectorAll('.image-options img.selected').forEach(img => img.classList.remove('selected'));
      const customInput = document.getElementById('custom-image'); if (customInput) customInput.value = '';
      ...
      // 為網路圖片生成標識符和顯示名稱
      currentImageIdentifier = 'net_' + sanitizeFilenameForKey(imageName); // 使用清理函數
      currentImageDisplayName = imageName; // 直接使用 Pixabay 提供的名稱
      ...
      if (imageOptions) {
        const preview = document.createElement('img');
        preview.src = selectedImage; // 可以考慮用 previewUrl 加快顯示
        preview.alt = '網路圖片預覽';
  ...
  function initUI() {
    ...
    console.log("UI 初始化完成 (添加雙擊, 修復重置計時, 區分自定義記錄)");
  }
